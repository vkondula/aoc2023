...
class Brick:
    ...
    def moved_up(self) -> Optional[set[tuple[int, int, int]]]:
        return set(
            (x, y + 1, z)
            for x, y, z in self.parts
        )
    ...
    for brick_id, supporting in supports.items():
        for supporting_id in supporting:
            if len(supported_by[supporting_id]) == 1:
                candidates.add(brick_id)
                break
    total = 0
    for candidate in candidates:
        new_supports = deepcopy(supports)
        new_supported_by = deepcopy(supported_by)
        stack = [candidate]
        while stack:
            brick_id = stack.pop()
            for supporting_id in new_supports[brick_id]:
                new_supported_by[supporting_id].remove(brick_id)
                if not new_supported_by[supporting_id]:
                    total += 1
                    stack.append(supporting_id)
    print(total)
    return total
